...
public class HeapPQ{
	...
	public AdjNode[] heap;  //the heap is implemented using an array so that we can get constant time access to each AdjNode
	...
	//------------------------------------------------------------------------------
	//add
	public void add(AdjNode vertex){
		if(heap.length == logSize){
			upSize();  //increase the capacity of the heap if it is full
		}
		
		heap[logSize] = vertex; //add the new AdjNode to the heap
		indirectionTable.add(String.valueOf(vertex.getVertexNumber()), logSize); //add new AdjNode to indirection table
	...
	//-------------------------------------------------------------------------------
	//remove
	// public void remove(String vertexNum){
	// 	int indexOfAdjNodeToRemove = this.getIndexOfAdjNode(vertexNum);
	// 	remove(indexOfAdjNodeToRemove);
	// }
	...
	public void remove(int removalIndex){
		// System.out.println("!!!!!!!!!!!!!!!!!! removalIndex: " + removalIndex);
		indirectionTable.add(String.valueOf(heap[removalIndex].getVertexNumber()), -1); //get the vin of the AdjNode we want to remove, access its path in the indirection table, mark this AdjNode's index as -1 to represent that it isn't part of the heap anymore
		if(removalIndex != logSize-1){ //only perform the following move if removalIndex is not the last index where an object is stored
			...
			indirectionTable.add(String.valueOf(heap[removalIndex].getVertexNumber()), removalIndex); //update the index of the object we placed into the removalIndex 
		}
	...
	public void swim(int childIndex){
		...
		// if(priorityField == 0){  //priorityField == 0 means that comparing based on price
			boolean childLess = heap[parentIndex].getLatency() > heap[childIndex].getLatency(); //determines if parent is higher priority than child
			...
			while(childIndex > 0 && childLess){
				...
				AdjNode temp = heap[parentIndex];
				heap[parentIndex] = heap[childIndex];
				heap[childIndex] = temp;
				//update the indicies of the swapped objects in the indirection table
				indirectionTable.add(String.valueOf(heap[parentIndex].getVertexNumber()), parentIndex);
				indirectionTable.add(String.valueOf(heap[childIndex].getVertexNumber()), childIndex);
				...
				childLess = heap[parentIndex].getLatency() > heap[childIndex].getLatency(); //update childLess
			}
		// }
		...
		// 		heap[childIndex] = temp;
		// 		//update the indicies of the swapped objects in the indirection table
		// 		indirectionTable.add(heap[parentIndex].getVertexNumber(), parentIndex);
		// 		indirectionTable.add(heap[childIndex].getVertexNumber(), childIndex);

		// 		childIndex = parentIndex; //update childIndex, child is where parent used to be (moved the AdjNode object we are calling swim() on to this position)
		// 		parentIndex = (childIndex-1)/2; //update parentIndex, new parent is at childIndex/2
	...
	public void sink(int parentIndex){
		...
		// if(priorityField == 0){  //comparing based on price
			while(rightChildIndex < logSize){  //starting array at index 0, so don't want it to be equal to logSize
				...
				lcLess = heap[leftChildIndex].getLatency() < heap[rightChildIndex].getLatency();
				if(lcLess){
				...
				comparatorLess = heap[parentIndex].getLatency() > heap[comparator].getLatency();
				...
				//update the indicies of the swapped objects in the indirection table
				indirectionTable.add(String.valueOf(heap[parentIndex].getVertexNumber()), parentIndex);
				indirectionTable.add(String.valueOf(heap[comparator].getVertexNumber()), comparator);


				parentIndex = comparator; //update parentIndex,  (moved the AdjNode we are calling sink() on to this position)
			...
			if(leftChildIndex < logSize && heap[leftChildIndex] != null){ //case for if left child != null, but right child == null
				...
				if(heap[leftChildIndex].getLatency() < heap[parentIndex].getLatency()){ //must test to see if need to swap parent and left child
					...
					//update the indicies of the swapped objects in the indirection table
					indirectionTable.add(String.valueOf(heap[parentIndex].getVertexNumber()), parentIndex);
					indirectionTable.add(String.valueOf(heap[leftChildIndex].getVertexNumber()), leftChildIndex);
				}
			}
		// }
		...
		// 		//update the indicies of the swapped objects in the indirection table
		// 		indirectionTable.add(heap[parentIndex].getVertexNumber(), parentIndex);
		// 		indirectionTable.add(heap[comparator].getVertexNumber(), comparator);
		...
		// 			//update the indicies of the swapped objects in the indirection table
		// 			indirectionTable.add(heap[parentIndex].getVertexNumber(), parentIndex);
		// 			indirectionTable.add(heap[leftChildIndex].getVertexNumber(), leftChildIndex);
		// 		}
		// 	}
		// }
	}
	...
	public int getIndexOfAdjNode(String vertexNum){
		return indirectionTable.contains(vertexNum);
	}
	...
	private void upSize(){  
		AdjNode[] biggerHeap = new AdjNode[heap.length * 2]; // creates a new deque twice the length of the full deque
